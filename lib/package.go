...
func (pkg PackageContext) GeneratePackageInfo() {
	...
	err = ioutil.WriteFile(filepath.Join(home, "alpmbuild/package", ".PKGINFO"), []byte(packageInfo), 0644)
	if err != nil {
		panic(fmt.Sprintf("Failed to generate pkginfo:\n%s", err.Error()))
	}
}
...
func setupDirectories() error {
	...
	for _, dir := range []string{"alpmbuild/buildroot", "alpmbuild/package", "alpmbuild/sources", "alpmbuild/packages"} {
...
func (pkg PackageContext) CompressPackage() {
	...
	cmd := exec.Command("sh", "-c", "tar -czvf ../packages/"+pkg.Name+".pkg.tar.gz * .PKGINFO .MTREE")
	output, err := cmd.CombinedOutput()
	if err != nil {
